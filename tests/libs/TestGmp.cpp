...
using delpi::gmp::ceil;
using delpi::gmp::floor;
using delpi::gmp::StringToMpq;
using delpi::gmp::ToMpq;
using delpi::gmp::ToMpqClass;
using std::make_pair;
...
INSTANTIATE_TEST_SUITE_P(TestGmp, TestGmp,
                         ::testing::Values(make_pair("0", mpq_class{0}), make_pair(".", mpq_class{0}),
                                           ...
                                           make_pair("0/1010", mpq_class{0}), make_pair("", mpq_class{0}),
                                           make_pair("inf", mpq_class{1e100})));
...
TEST_P(TestGmp, ConvertStringToMpqSuffixOther) {
  auto [s, expected] = GetParam();
  std::string new_string = s + "abc and other stuff";
  expected.canonicalize();
  EXPECT_EQ(StringToMpq(std::string_view{new_string.data(), s.size()}), expected);
}