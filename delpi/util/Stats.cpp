...
#define DELPI_ITERATION_STATS_FMT "{:<35} @ {:<26} = {:>15}"
...
std::string Stats::ToSegmentString() const {
  return fmt::format(DELPI_STATS_FMT, operations_name_, class_name_, timer_.seconds());
}
...
std::string IterationStats::ToSegmentString() const {
  return fmt::format(DELPI_ITERATION_STATS_FMT, iterations_name_, class_name_, iterations_.load());
}
...
IterationStats::IterationStats(const delpi::IterationStats &other)
