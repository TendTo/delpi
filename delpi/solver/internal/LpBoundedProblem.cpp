...
void LpProblem::SlackForm(Matrix<T>& slack_A, Vector<T>& slack_b, Vector<T>& slack_c) const {
  ...
  Vector<T> slack_lb{Vector<T>::Constant(num_rows_ + num_ub, gmp::infinity)};
  Vector<T> slack_ub{Vector<T>::Constant(num_rows_ + num_ub, gmp::infinity)};
  ...
  for (const auto& [idx, value] : x_ub) {
    slack_lb(num_rows_ + idx) = -value;
    slack_ub(num_rows_ + idx) = 0;
  }
  ...
  // Set the rhs
  slack_b = Vector<T>{num_columns_ + num_rows_};
  slack_b.head(num_columns_) = x_lb;
...
void LpProblem::BoundedVariableForm(Matrix<T>& bounded_A, Vector<T>& bounded_c, Vector<T>& lb, Vector<T>& ub) const {
  DELPI_TRACE("LpProblem::BoundedVariableForm()");
  bounded_A = Matrix<T>(num_rows_, num_columns_ + num_rows_);
  bounded_A.leftCols(num_columns_) = A_;
  bounded_A.rightCols(num_rows_).setIdentity();

  // Compute the bounds on the slack variables
  lb = Vector<T>::Constant(num_columns_ + num_rows_, gmp::ninfinity);
  ub = Vector<T>::Constant(num_columns_ + num_rows_, gmp::infinity);
  lb.head(num_columns_) = x_lb_;
  for (const auto& [idx, value] : x_ub_) ub(idx) = value;
  for (Index i = 0; i < num_rows_; ++i) {
    DELPI_ASSERT(!gmp::IsInfinity(b_lb_(i) || !gmp::IsInfinity(b_ub_(i))), "At least one bound must be finite");
    if (!gmp::IsInfinity(b_lb_(i))) lb(num_columns_ + i) = -b_lb_(i);
    if (!gmp::IsInfinity(b_ub_(i))) ub(num_columns_ + i) = -b_ub_(i);
  }

  bounded_c = Vector<T>::Zero(num_columns_ + num_rows_);
  bounded_c.head(num_columns_) = c_;
}
