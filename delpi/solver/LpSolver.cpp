...
LpSolver::ColumnIndex LpSolver::AddVariable(const Variable& var) {
  ...
  return AddVariable(var, 0, infinity_, 0);
}
LpSolver::ColumnIndex LpSolver::AddVariable(const Variable& var, const mpq_class& obj) {
  DELPI_ASSERT(!var_to_col_.contains(var), "Variable already exists in the LP.");
  // Add a column representing this variable to the lp solver
  return AddVariable(var, 0, infinity_, obj);
}
LpSolver::ColumnIndex LpSolver::AddVariable(const Variable& var, const mpq_class& lb, const mpq_class& ub) {
  DELPI_ASSERT(!var_to_col_.contains(var), "Variable already exists in the LP.");
  // Add a column representing this variable to the lp solver
  return AddVariable(var, lb, ub, 0);
}
...
LpResult LpSolver::Optimise(mpq_class& precision, const bool store_solution) {
  DELPI_ASSERT(solution_.empty() && dual_solution_.empty(), "Previous run information was not backtracked.");
